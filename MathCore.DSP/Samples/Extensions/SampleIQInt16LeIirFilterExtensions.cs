...
/// <summary>Методы БИХ-фильтрации для interleaved IQ-потока int16 little-endian (USRP B210)</summary>
public static class SampleIQInt16LeIirFilterExtensions
{
    ...
    public static void FilterIIRInterleavedInt16Le(
    ...
    public static void FilterIIRInterleavedInt16Le(
        ...
        double[] B,
        double[] StateI,
        double[] StateQ) => FilterIIRInterleavedInt16Le((ReadOnlySpan<byte>)Samples, Samples, A, B, StateI, StateQ);
    ...
    public static byte[] FilterIIRInterleavedInt16Le(this ReadOnlySpan<byte> Samples, double[] A, double[] B)
    {
        ArgumentNullException.ThrowIfNull(A);

        var result = new byte[Samples.Length];
        Samples.FilterIIRInterleavedInt16Le(result, A, B, new double[A.Length], new double[A.Length]);
        return result;
    }
    ...
    public static long FilterIIRInterleavedInt16Le(
        this Stream Source,
        Stream Destination,
        byte[] Buffer,
        double[] A,
        ...
        double[] StateQ)
    {
        ...
        while (true)
        {
            ...
            if (tail_count > 0)
            {
                ...
                if (tail_count == 4)
                {
                    ProcessFrame(tail, frame_buffer, A, B, StateI, StateQ);
            ...
            if (block_count > 0)
            {
                ...
                FilterIIRInterleavedInt16Le(block, block, A, B, StateI, StateQ);
    ...
    /// <param name="B">Массив коэффициентов прямой связи</param>
    /// <returns>Количество записанных байт</returns>
    public static long FilterIIRInterleavedInt16Le(this Stream Source, Stream Destination, byte[] Buffer, double[] A, double[] B)
    {
        ArgumentNullException.ThrowIfNull(A);

        return FilterIIRInterleavedInt16Le(Source, Destination, Buffer, A, B, new double[A.Length], new double[A.Length]);
    }
    ...
    /// <returns>Количество записанных байт</returns>
    public static async Task<long> FilterIIRInterleavedInt16LeAsync(
        this Stream Source,
        Stream Destination,
        byte[] Buffer,
        double[] A,
    ...
    /// <param name="Cancellation">Маркер отмены операции</param>
    /// <returns>Количество записанных байт</returns>
    public static Task<long> FilterIIRInterleavedInt16LeAsync(
        this Stream Source,
        Stream Destination,
        byte[] Buffer,
        double[] A,
        ...
        CancellationToken Cancellation = default)
    {
        ...
        return FilterIIRInterleavedInt16LeAsync(Source, Destination, Buffer, A, B, new double[A.Length], new double[A.Length], Cancellation);
    }

    private static void ProcessFrame(ReadOnlySpan<byte> Input, Span<byte> Output, double[] A, double[] B, double[] StateI, double[] StateQ)
    {
